        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    await asyncio.sleep((attempt + 1) * 5)  # 递增延迟
                else:
                    print("所有连接尝试都失败了，请检查：")
                    print("1. 确保只有4个toio设备开机")
                    print("2. 关闭其他蓝牙设备")
                    print("3. 重启蓝牙适配器")
                    print("4. 建议使用3设备版本以获得更好的稳定性")
                    raise
                
    async def run(self):
        """运行主程序"""
        print("=== 4个Toio中断控制程序（优化版）===")
        print("注意：4设备同时连接可能不稳定，建议使用3设备版本")
        print("正在连接4个toio设备...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n程序错误: {e}")
        print("建议使用 multi_toio_interrupt_control_3devices.py 以获得更好的稳定性") 